...
export default function SCLApplicationSelectorPage() {
  return (
    <div className="flex-1 flex flex-col h-full bg-slate-50 dark:bg-slate-900 overflow-y-auto">
      <div className="bg-white dark:bg-slate-900 border-b border-gray-200 dark:border-slate-800 px-6 py-4 flex items-center justify-between shrink-0">
        <div>
          ...
          <h1 className="text-xl font-bold tracking-tight text-gray-900 dark:text-slate-100">Smart Credit Line (SCL)</h1>
          <p className="text-sm text-gray-500 dark:text-slate-400">Select the customer profile type for the revolving credit line application.</p>
        </div>
      </div>
      ...
      <div className="p-6 max-w-4xl mx-auto w-full mt-10">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {/* SCL Personal */}
          <Link 
            href="/dashboard/applications/new/scl/personal"
          ...
          >
            ...
            <h3 className="text-lg font-bold text-gray-900 dark:text-slate-100 mb-2">SCL Personal</h3>
            <p className="text-sm text-gray-500 dark:text-slate-400 mb-6 flex-1">
              Apply for a revolving credit line for individual borrowers (salaried or self-employed).
            </p>
            <span className="inline-flex items-center justify-between w-full bg-black dark:bg-white text-white dark:text-black px-4 py-2.5 rounded-md text-sm font-medium hover:bg-gray-800 dark:hover:bg-gray-200 transition-colors">
              Start SCL Personal <ArrowRight className="w-4 h-4" />
            </span>
          </Link>

          {/* SCL Business */}
          <Link 
            href="/dashboard/applications/new/scl/business"
            className="bg-white dark:bg-slate-900 border border-gray-200 dark:border-slate-800 rounded-lg p-6 hover:border-black dark:hover:border-white hover:shadow-md transition-all cursor-pointer group flex flex-col h-full"
          >
            ...
            <h3 className="text-lg font-bold text-gray-900 dark:text-slate-100 mb-2">SCL Business</h3>
            <p className="text-sm text-gray-500 dark:text-slate-400 mb-6 flex-1">
              Apply for a business Smart Credit Line for entities (Pvt Ltd, Public Ltd, Partnership, LLP, or Proprietorship).
            </p>
            <span className="inline-flex items-center justify-between w-full bg-white dark:bg-slate-800 border border-gray-300 dark:border-slate-700 text-gray-700 dark:text-slate-200 px-4 py-2.5 rounded-md text-sm font-medium hover:bg-gray-55 dark:hover:bg-slate-700 transition-colors shadow-sm">
              Start SCL Business <ArrowRight className="w-4 h-4" />
            </span>
          </Link>
        </div>
      </div>
