...
import LapLoanApplicationForm from "@/components/LapLoanApplicationForm";
import { ArrowLeft } from "lucide-react";
import Link from "next/link";

export default function LAPPersonalApplicationPage() {
      ...
      <div className="flex-1 flex flex-col h-full bg-white dark:bg-slate-955 overflow-y-auto">
        <div className="flex-1 flex items-center justify-center p-6">
          <div className="max-w-md w-full text-center">
            ...
            <h2 className="text-2xl font-bold text-gray-900 dark:text-slate-100 mb-2">LAP Application Submitted!</h2>
            <p className="text-gray-600 dark:text-slate-400 mb-6">Your Loan Against Property (Personal) application has been submitted and is under review.</p>
  ...
  return (
    <div className="flex-1 flex flex-col h-full bg-slate-50 dark:bg-slate-900 overflow-y-auto">
      <div className="bg-white dark:bg-slate-900 border-b border-gray-200 dark:border-slate-800 px-4 py-2 shrink-0 flex justify-between items-center">
        <div>
          <Link href="/dashboard/applications/new/lap" className="inline-flex items-center gap-1 text-[11px] font-bold text-gray-600 dark:text-slate-400 hover:text-gray-900 dark:hover:text-slate-100 mb-1">
            <ArrowLeft className="w-3 h-3" /> Back
          </Link>
          <h1 className="text-lg font-bold tracking-tight text-gray-900 dark:text-slate-100">Loan Against Property</h1>
          <p className="text-xs text-gray-500 dark:text-slate-400 mt-0.5">Complete all steps to apply for a personal loan against property collateral.</p>
        </div>
        <div>
